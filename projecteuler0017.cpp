...
one, two, three, four, five, then there are 19letters used in total
if all the numbers from 1 to 1000 inclusive were written out in words,
how many letters would be used?


*/
vs num = {"", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"};
vs tens = {"a", "a", "twenty", "thirry", "fourty", "fifty", "sixty", "seventy", "eighty", "ninety"};
vs tt {"ten", "eleven", "twelve", "thirteen", "fourteen", "fifteen", "sixteen", "seventeen", "eighteen", "nineteen"};
int countLetters(int n) {
    int sum = 0;
    if (n == 1000) return 3+8;
    if (n > 100) {
        sum += num[n/100].size() + 7;
        n %= 100;
    }
    sum += 3;
    if (n>20) {
